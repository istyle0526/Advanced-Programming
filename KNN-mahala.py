...
for k in range (1,12,2):
    KNN_R = KNeighborsClassifier(n_neighbors=k, metric = 'mahalanobis', metric_params = {'V' : train_x.cov()})
...
fig, ax = plt.subplots()
ax.plot(range(1,12,2),train_gmean, color ='black', marker='o', label = 'G-mean of train')
ax.plot(range(1,12,2),test_gmean, color ='blue', marker='x', label = 'G-mean of test')
plt.legend()
...
KNN_O = KNeighborsClassifier(n_neighbors=optimal_k, metric = 'mahalanobis', metric_params = {'V' : train_x.cov()})
...
print(C_2)
print("Accuracy using test data is: %.3f"%(Optimal_accuracy_2))
