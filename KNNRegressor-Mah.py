...
for k in range (1,20,2):
    KNN_R = KNeighborsRegressor(n_neighbors=k, metric = 'mahalanobis', metric_params = {'V' : train_x.cov()})
...
KNN_R = KNeighborsRegressor(n_neighbors=optimal_k, metric = 'mahalanobis', metric_params = {'V' : train_x.cov()})
KNN_R.fit(train_x, train_y)
estimates = KNN_R.predict(train_x)
estimates2 = KNN_R.predict(test_x)
